...
class Poker:
    ...
    def play(self, history, prob, sample_prob) -> int:
        self.ctr += 1
        round = len(history)
        hero = round % 2
        villian = 1 - hero
        ...
        if self.check_terminal(history) != 3:
            return self.check_terminal(history) / sample_prob, 1

        information = str(self.cards[hero]) + history
        strategy = self.get_strategy(information, prob[hero])
        epsilon = 0.1
        strategy_with_epsilon = [0,0]
        for i in range(2):
            strategy_with_epsilon[i] = epsilon / len(strategy) + (1 - epsilon) * strategy[i]

        t1 = time.time()

        util = 0
        act = np.random.choice([0,1], p=strategy_with_epsilon)
        new_history = history + self.action[act]
        probability = strategy[act]
        new_prob = prob.copy()
        new_prob[hero] *= probability
        util, p_tail = self.play(new_history, new_prob, sample_prob * strategy_with_epsilon[act])
        util *= -1

        weight = util * prob[villian]
        for i in range(len(strategy)):
            regret = 0
            if i == act:
                regret = weight * (1.0 - strategy[act]) * p_tail
            else:
                regret = -weight * p_tail * strategy[act]
            self.node_map[information].regret_sum[i] += regret
        # for i in range(2):
